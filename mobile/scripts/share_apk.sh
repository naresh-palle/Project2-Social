#!/usr/bin/env bash
# Copy the latest release APK to local dist, Cursor artifacts, and frontend/public
# (GitHub Pages picks it up on the next `npm run deploy:gh-pages`).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
REPO="$(cd "$ROOT/.." && pwd)"
OUT="$ROOT/build/app/outputs/flutter-apk/app-release.apk"
if [ ! -f "$OUT" ]; then
  echo "No APK at $OUT — run bash mobile/scripts/build_apk.sh first." >&2
  exit 1
fi
VERSION="$(awk '/^version:/{print $2; exit}' "$ROOT/pubspec.yaml" | cut -d+ -f1)"
NAME="cr8-studio-release.apk"
VERSIONED="CR8-Studio-${VERSION}.apk"
...
if [ -d /opt/cursor/artifacts ]; then
  ...
  cp -f "$OUT" "/opt/cursor/artifacts/CR8-Studio.apk"
fi
